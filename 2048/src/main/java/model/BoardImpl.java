...
public class BoardImpl extends Observable implements Board {
    ...
    Map<String, Boolean> hashTileUsed = new HashMap<>();
    ...
    public void commit() {
        ...
        String position = getTilePosition();
        int x = Integer.parseInt(position.substring(0, 1));
        int y = Integer.parseInt(position.substring(1, 2));

        currentBoard[x][y] = new TileImpl(new Random().nextDouble() < 0.9 ? 2 : 4);
        nextBoard = new Tile[sideSizeInSquares][sideSizeInSquares];
        setChanged();
    ...
    public void loadBoard(int[][] rankMatrix) {
        for (int i = 0; i < sideSizeInSquares; i++) {
            for (int j = 0; j < sideSizeInSquares; j++) {
                ...
                hashTileUsed.put("" + i + j, rankMatrix[i][j] > 0);
            }
        }
    }
    ...
    private String getTilePosition() {
        String res = "";
        Set<String> keySet = hashTileUsed.keySet();
        Iterator it = keySet.iterator();
        boolean isUsed = false;
        while (!isUsed && it.hasNext()) {
            isUsed = hashTileUsed.get(it.next());
        }
